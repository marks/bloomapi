    ...
    listener: function(data){
      ...
      for(var page in data['*']){
        ...
        Object.keys(pageData).map(function(label){
          ga.trackEvent('API', action, label, pageData[label], undefined, page,  false )
        })
      }
...
var app = connect()
  ...
  .use(function(req, res, next){
    ...
    res.on('header', function(){
      ...
      ga.trackEvent('API', 'General', 'Response Time', res.getHeader('X-Response-Time'), undefined, req.headers.originalUrl, false)
    });
  })
...
rest.get('/search', function api_search(req,content,callback){
  ga.trackPage('API - Search', req.headers.originalUrl, function (err, resp) {});
  ...
  var sqlQueryStartTime = Date.now()
    ...
    // todo - event needs to be tied to current page/query!
    ga.trackEvent('API', 'Search - DB', 'PgSQL Query Time', sqlQueryTime, undefined, req.headers.originalUrl, false)
